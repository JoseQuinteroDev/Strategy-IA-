...
from hybrid_quant.baseline.orb_frequency_expansion import (
    OrbFrequencyExpansionRunner,
    load_orb_frequency_expansion_config,
)
...
"""
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            experiment_path = tmp_path / "orb_frequency_expansion_test.yaml"
            experiment_path.write_text(experiment_yaml, encoding="utf-8")

            runner = OrbFrequencyExpansionRunner(
                config_dir,
                load_orb_frequency_expansion_config(experiment_path),
            )
            artifacts = runner.run(
                input_frame=_orb_frame(),
                output_dir=tmp_path / "orb_frequency_expansion",
                allow_gaps=True,
            )
            ...
            self.assertEqual(payload["experiment_name"], "orb_frequency_expansion_test")
            self.assertEqual(len(payload["variants"]), 2)
            self.assertIn("best_frequency_variant", payload["conclusion"])
            self.assertIn("best_balance_variant", payload["conclusion"])

            results = pd.read_csv(artifacts.results_path)
            self.assertIn("trades_per_month_avg", results.columns)
            self.assertIn("pct_days_with_trade", results.columns)
            self.assertIn("passes_balance_gate", results.columns)
