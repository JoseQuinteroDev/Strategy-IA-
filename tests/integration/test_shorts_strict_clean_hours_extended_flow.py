...
from hybrid_quant.baseline.shorts_strict_clean_hours_extended import run_extended_research
...
class ShortsStrictCleanHoursExtendedFlowTests(unittest.TestCase):
    def test_extended_runner_generates_artifacts(self) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        config_dir = repo_root / "configs"

        with tempfile.TemporaryDirectory() as tmp_dir:
            temp_dir = Path(tmp_dir)
            input_path = temp_dir / "ohlcv.csv"
            output_dir = temp_dir / "shorts_extended"
            _contextual_frame().to_csv(input_path, index=True)

            artifacts = run_extended_research(
                config_dir=config_dir,
                experiment_config=config_dir / "experiments" / "shorts_strict_clean_hours_extended.yaml",
            ...
            self.assertTrue(artifacts.comparison_summary_path.exists())
            self.assertTrue(artifacts.yearly_summary_path.exists())
            self.assertTrue(artifacts.quarterly_summary_path.exists())
            self.assertTrue(artifacts.hourly_summary_path.exists())
            self.assertTrue(artifacts.side_summary_path.exists())
