...
from hybrid_quant.baseline.session_trend_30m_zoom import (
...
class SessionTrend30mZoomFlowTests(unittest.TestCase):
    def test_zoom_runner_generates_artifacts(self) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        config_dir = repo_root / "configs"
        experiment = load_session_trend_30m_zoom_config(
            config_dir / "experiments" / "session_trend_30m_zoom.yaml"
        )
        runner = SessionTrend30mZoomRunner(config_dir, experiment)

        with tempfile.TemporaryDirectory() as tmp_dir:
            output_dir = Path(tmp_dir) / "session_trend_zoom"
            ...
            self.assertTrue(artifacts.temporal_summary_path.exists())
            self.assertTrue(artifacts.filter_ablation_summary_path.exists())
